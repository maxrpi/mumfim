...
namespace mumfim
{
  namespace ml
  {
    ...
    at::Tensor ComputeRightCauchyGreen(at::Tensor & deformation_gradient)
    {
    ...
    std::pair<int, int> Tensor4Index(int i)
    {
      if (i == 0)
      {
        return {0, 0};
    ...
    torch::Tensor Tensor4ToVoigt(const torch::Tensor & tensor)
    {
    ...
    torch::Tensor VoigtToTensor4(const torch::Tensor & voigt)
    {
      using torch::indexing::None;
      using torch::indexing::Slice;
    ...
    torch::Tensor TotalLagrangianStiffnessToULStiffness(
    ...
    at::Tensor MandelProbeInv(int batch_size)
    {
    ...
    at::Tensor ProbeDirections(int batch_size)
    {
    ...
    torch::Tensor ComputeMaterialStiffness(
    ...
    torch::Tensor PK2ToCauchy(const torch::Tensor & PK2,
    ...
    DerivativeResult ComputeCauchyStressStiffness(
        ...
        const at::Tensor & right_cauchy_green)
    {
      ...
      auto material_stiffness =
          ComputeMaterialStiffness(PK2, right_cauchy_green);
      auto val_tensor = VoigtToTensor4(material_stiffness);
      result.material_stiffness = TotalLagrangianStiffnessToULStiffness(
          val_tensor, deformation_gradient);
      return result;
    }
    ...
    apf::DynamicMatrix StressToApfDynamicMatrix(
    ...
    apf::DynamicMatrix TorchTensorToApfDynamicMatrix(
